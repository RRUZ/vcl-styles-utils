...
procedure _WriteText(ACanvas: TCanvas; ARect: TRect; DX, DY: Integer; const AText: string; Alignment: TAlignment;
...
begin
  ...
  case Alignment of
    taLeftJustify:
      X := ARect.Left + DX;
    taRightJustify:
      X := ARect.Right - ACanvas.TextWidth(AText) - 3;
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        try
          ...
          MultiSelected := (dgMultiSelect in Options) and DataLink.Active and
      ...
      begin
        ...
        if LStyleServices.Enabled and not LStyleServices.IsSystemStyle then
          ...
          Indicators.Draw(Canvas, X, (ARect.Top + ARect.Bottom - Indicators.Height) shr 1,
    ...
    begin
      ...
      if not CurrentColumn.Showing then
        exit;
      ...
      begin
        ...
        try
          ...
          if DefaultDrawing then
            _WriteText(Canvas, ARect, 3, 2, Value, CurrentColumn.Alignment,
              UseRightToLeftAlignmentForField(CurrentColumn.Field, CurrentColumn.Alignment));
        ...
        if DefaultDrawing and (gdSelected in AState) and ((dgAlwaysShowSelection in Options) or Focused) and
          not(csDesigning in ComponentState) and not(dgRowSelect in Options) and (UpdateLock = 0) and
          (ValidParentForm(Self).ActiveControl = Self) and (FInternalDrawingStyle = gdsThemed) and
  ...
  if (gdFixed in AState) and ([dgRowLines, dgColLines] * Options = [dgRowLines, dgColLines]) and
    (FInternalDrawingStyle = gdsClassic) and not(gdPressed in AState) then
    Winapi.Windows.InflateRect(ARect, 1, 1);
end;

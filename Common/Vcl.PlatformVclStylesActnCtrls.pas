...
type
  ...
  public
    function GetControlClass(ActionBar: TCustomActionBar; AnItem: TActionClientItem)
      : TCustomActionControlClass; override;
...
function DoDrawText(DC: HDC; Details: TThemedElementDetails; const S: string; var R: TRect; Flags: TTextFormat;
...
function InternalDrawText(DC: HDC; Details: TThemedElementDetails; const S: string; var R: TRect; Flags: TTextFormat;
  Color: TColor = clNone): Boolean;
var
  // LColor: TColor;
...
function TPlatformVclStylesStyle.GetControlClass(ActionBar: TCustomActionBar; AnItem: TActionClientItem)
...
begin
  ...
  begin
    ...
    else if (AnItem.Action is TStaticListAction) or (AnItem.Action is TVirtualListAction) then
...
procedure TThemedMenuItemEx.NativeDrawText(DC: HDC; const Text: string; var Rect: TRect; Flags: Integer);
const
...
procedure TThemedMenuItemEx.DrawText(var Rect: TRect; var Flags: Cardinal; Text: string);
var
  LRect: TRect;
begin
  if Selected and Enabled then
  ...
  OffsetRect(LRect, Rect.Left, ((PaintRect.Bottom - PaintRect.Top) - (LRect.Bottom - LRect.Top)) div 2);
...
{ TThemedMenuButtonEx }
procedure TThemedMenuButtonEx.NativeDrawText(const Text: string; var Rect: TRect; Flags: Integer);
const
...
const
  MenuStates: array [Boolean, Boolean] of TThemedMenu = ((tmMenuBarItemNormal, tmMenuBarItemPushed),
    (tmMenuBarItemHot, tmMenuBarItemPushed));
begin
  Canvas.Brush.Color := ActionBar.ColorMap.Color;
  StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(MenuStates[MouseInControl, Selected]),
    PaintRect);
end;

procedure TThemedMenuButtonEx.DrawText(var ARect: TRect; var Flags: Cardinal; Text: string);
var
  LRect: TRect;
begin
...
var
  SaveIndex: Integer;
begin
  if not StyleServices.IsSystemStyle and ActionClient.Separator then
    Exit;
  ...
  try
    ...
    begin
      if (MouseInControl or IsChecked) and Assigned(ActionClient) { and not ActionClient.Separator) } then
      begin
        StyleServices.DrawElement(Canvas.Handle,
          StyleServices.GetElementDetails(CheckedState[IsChecked or (FState = bsDown)]), PaintRect);

        if not MouseInControl then;
        // StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(ttbButtonPressed), PaintRect);
      end
      else; // StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(ttbButtonNormal), PaintRect);
    end
    else
...
begin
  ...
  try
    if Enabled and not(ActionBar.DesignMode) then
    begin
      if (MouseInControl or IsChecked or DroppedDown) and (Assigned(ActionClient) and not ActionClient.Separator) then
      begin
        StyleServices.DrawElement(Canvas.Handle,
          StyleServices.GetElementDetails(CheckedState[IsChecked or (FState = bsDown)]), PaintRect);
    ...
    else; // StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(DisabledState[IsChecked]), PaintRect);
  finally

...
type
  ...
  /// <summary> Form Style hook to draw native windows buttons
  /// </summary>
  TFormStyleHookNativeButtons=class(TFormStyleHook)
  protected
    procedure PaintNC(Canvas: TCanvas); override;
  end;
...
{ TFormStyleHookNativeButtons }

procedure TFormStyleHookNativeButtons.PaintNC(Canvas: TCanvas);
var
  Details, CaptionDetails, IconDetails: TThemedElementDetails;
  Detail: TThemedWindow;
  R, R1, DrawRect, ButtonRect, TextRect: TRect;
...
begin
  ...
  begin
    ...
    //StyleServices.DrawElement(CaptionBuffer.Canvas.Handle, Details, ButtonRect);
    //DrawFrameControl(CaptionBuffer.Canvas.Handle, ButtonRect, DFC_CAPTION, DFCS_CAPTIONMIN);
    ...
    if hTheme <> 0 then
      try
        ...
        if IsThemeBackgroundPartiallyTransparent(hTheme, iPartId, iStateId) then
          DrawThemeParentBackground(hTheme, CaptionBuffer.Canvas.Handle, @ButtonRect);
       {
       ButtonRect.width:=GetThemeSysSize(hTheme, SM_CXSIZE);
       ButtonRect.height:=GetThemeSysSize(hTheme, SM_CYSIZE);
       }
        DrawThemeBackground(hTheme, CaptionBuffer.Canvas.Handle, iPartId, iStateId,  ButtonRect, nil);
      finally
